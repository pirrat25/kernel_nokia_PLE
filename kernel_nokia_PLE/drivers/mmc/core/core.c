...
void mmc_set_ios(struct mmc_host *host)
{
	...
	if(!host->num_cq_slots && ios->old_rate != ios->clock)
		pr_err("%s %s clock=%u\n", mmc_hostname(host),__func__, ios->clock);
				...
				bool cd_irq)
{
	...
	if(!host->num_cq_slots)
		pr_err("%s %s delay=%lu cd_irq=%d, man_resume=%d, ignore_bus_resume=%d\n",
			__func__, mmc_hostname(host), delay,cd_irq, 
					...
					unsigned long mode, void *unused)
{
	...
	case PM_POST_RESTORE:
		...
		if(!host->num_cq_slots)
			pr_err("%s %s manual_resume=%d, ignore_bus_resume_flag=%d\n", 
				mmc_hostname(host), __func__, mmc_bus_manual_resume(host),
				host->ignore_bus_resume_flags);
