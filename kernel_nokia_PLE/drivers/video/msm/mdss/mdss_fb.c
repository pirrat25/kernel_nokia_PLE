...
/* FIHTDC - gatycclu - MF3-79 - Fix FTM backlight cmd failure problem { */
	if (strnstr(saved_command_line, "androidboot.mode=2", strlen(saved_command_line))) {
		pr_info("mfd->unset_bl_level set initial brightness as 255 for ftm\n");
		MDSS_BRIGHT_TO_BL(mfd->unset_bl_level, 255,
		mfd->panel_info->bl_max, mfd->panel_info->brightness_max);
	}
/* FIHTDC - gatycclu - MF3-79 - Fix FTM backlight cmd failure problem } */
